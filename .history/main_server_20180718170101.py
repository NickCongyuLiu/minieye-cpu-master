        ...
        for line in info:
            if ('Output info' in line):
                ...
                if(takeOrNot):
                    ...
                    if(int(meta_data.split('')[0])):
                        data_set.append([int(meta_data.split(' ')[0]),int(meta_data.split(' ')[1]),'lane_alg_fps'])
                    # data_set.append((line.split('lane.alg_fps')[1].strip().split(' '),))
            elif ('lane.fps' in line):
                if(takeOrNot):
                    meta_data = line.split('lane.fps')[1].strip()
                    data_set.append([int(meta_data.split(' ')[0]),int(meta_data.split(' ')[1]),'lane_fps'])
