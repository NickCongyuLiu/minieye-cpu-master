    ...
    def data_cleaning(self,origin_data):
        ...
        for line in info:
            if ('Output info' in line):
                ...
                if(takeOrNot):
                    CPU_timeStamp.append((line.split(':')[1].strip()))
            elif ('mtktscpu :' in line):
                if(takeOrNot):
                    # CPU_temp+=(line.split(':')[1].strip())+' '
                    CPU_temp.append((line.split(':')[1].strip()))
            elif ('CPU#0:' in line):
                if(takeOrNot):
                    CPU_one_freq.append((line.split(':')[1].strip()))
            elif ('CPU#4:' in line):
                if(takeOrNot):
                    CPU_two_freq.append((line.split(':')[1][1:6]))
