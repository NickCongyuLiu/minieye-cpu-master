    ...
    def data_cleaning(self,origin_data):
        ...
        for line in info:
            if ('Output info' in line):
                ...
                if(takeOrNot):
                    data_set.append(([line.split('lane.alg_fps')[1].strip(),'lane_alg_fps']))
            elif ('lane.fps' in line):
                if(takeOrNot):
                    data_set.append(([line.split('lane.fps')[1].strip(),'lane_fps']))
            elif ('vehicle.alg_fps' in line):
                if(takeOrNot):
                    data_set.append(([line.split('vehicle.alg_fps')[1].strip(),'vehicle_alg_fps']))
            elif ('v1.process_fps' in line):
                if(takeOrNot):
                    data_set.append(([line.split('v1.process_fps')[1].strip(),'v1_process_fps']))        
