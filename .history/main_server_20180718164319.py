    ...
    def data_cleaning(self,origin_data):
        ...
        for line in info:
            if ('Output info' in line):
                ...
                if(takeOrNot):
                    CPU_timeStamp.append(int(line.split(':')[1].strip()),'hello')
                ...
                if(takeOrNot):
                    CPU_power.append(int(line.split('current power =')[1].strip()[0:4]))
            elif ('lane.alg_fps' in line):
                if(takeOrNot):
                    lane_alg_fps.append((line.split('lane.alg_fps')[1].strip()))
            elif ('lane.fps' in line):
                if(takeOrNot):
                    lane_fps.appedn((line.split('lane.fps')[1].strip()))
            elif ('vehicle.alg_fps' in line):
                if(takeOrNot):
                    vehicle_alg_fps.append((line.split('vehicle.alg_fps')[1].strip()))
            elif ('v1.process_fps' in line):
                if(takeOrNot):
                    v1_process_fps.append((line.split('v1.process_fps')[1].strip()))        
