            ...
            if ('Output info' in line):
                ...
                if(takeOrNot):
                    ...
                    if(meta_data_arr[0] =='fps'):
                        data_set.append([int(meta_data_arr[1]),int(meta_data_arr[2][0:2]),'lane_alg_fps'])
                    else:
                        data_set.append([int(meta_data_arr[0]),int(meta_data_arr[1]),'lane_alg_fps'])

            elif ('lane.fps' in line):
                if(takeOrNot):
                    meta_data = line.split('lane.fps')[1].strip()
                    meta_data_arr = meta_data.split(' ')
                    data_set.append([int(meta_data_arr[0]),int(meta_data_arr[1][0:2]),'lane_fps'])
                ...
                if(takeOrNot):
                    ...
                    meta_data_arr = meta_data.split(' ')
                    data_set.append([int(meta_data_arr[0]),int(meta_data_arr[1][0:2]),'vehicle_alg_fps'])
            elif ('v1.process_fps' in line):
                if(takeOrNot):
                    meta_data = line.split('v1.process_fps')[1].strip()
                    meta_data_arr = meta_data.split(' ')
                    data_set.append([int(meta_data_arr[0]),float(meta_data_arr[1][0:2]),'v1.process_fps'])     
